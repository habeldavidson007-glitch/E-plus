...
export function validate(ast: AST.Program, config: Partial<ValidatorConfig> = {}): ValidationIssue[] {
...
function validateStatement(
    ...
    switch (stmt.type) {
        case 'Assignment':
            validateExpression(stmt.expression, stmt.line, 'assignment', issues, config);
            break;
        ...
        case 'Output':
            validateExpression(stmt.expression, stmt.line, 'output', issues, config);
            break;
        ...
        case 'Repeat':
            validateBlock(stmt.block, issues, config);
            if (stmt.iterable) {
                validateExpression(stmt.iterable, stmt.line, 'repeat iterable', issues, config);
            }
            break;
        ...
        case 'FunctionCall':
            ...
            for (const arg of stmt.arguments) {
                validateExpression(arg, stmt.line, 'function argument', issues, config);
            }
            break;

        case 'Return':
            validateExpression(stmt.expression, stmt.line, 'return', issues, config);
            break;
...
function validateBlock(block: AST.Block, issues: ValidationIssue[], config: ValidatorConfig): void {
...
function validateCondition(
    ...
    validateExpression(condition, line, 'condition', issues, config);
}
...
export function validateWithDefaults(ast: AST.Program): ValidationIssue[] {
