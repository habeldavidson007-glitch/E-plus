...
exports.validateWithDefaults = validateWithDefaults;
var IssueSeverity;
...
function validateStatement(validatorFn, statements) {
    for (const stmt of statements) {
        validatorFn(stmt);
    }
}
function validateStatementInternal(stmt, issues, config, depth) {
    // Check nesting depth
...
function calculateCognitiveScore(ast, issues) {
    const baseScore = 100;
...
function validateBlock(block, issues, config, depth = 0) {
    for (const stmt of block.statements) {
        validateStatementInternal(stmt, issues, config, depth);
    }
}
function validateExpression(expr, line, context, issues, config, ruleNumber) {
...
function countMathOperators(expr) {
    // Count: +, -, *, / (but not -> arrow)
...
function validateCondition(condition, line, issues, config) {
...
function countLogicalOperators(expr) {
    // Count: and, or, &&, ||
...
function validateWithDefaults(ast) {
